...
function renderFeaturedProperties(zoneKey) {
    ...
    if (renderedCount === 0) {
        ...
        if (customBanner && customBanner.enabled) {
            ...
            deck.innerHTML = `
                <div class="premium-corporate-banner glassmorphism" onclick="window.open('${bannerLink}', '_blank')" style="grid-column: 1 / -1; display: flex; flex-direction: column; justify-content: flex-end; padding: 35px 30px; border-radius: 12px; background: linear-gradient(180deg, rgba(8, 10, 15, 0.15) 0%, rgba(8, 10, 15, 0.75) 100%), url('${bannerPhoto}'); background-size: cover; background-position: center; border: 1.5px solid rgba(0, 240, 255, 0.35); box-shadow: 0 0 25px rgba(0, 240, 255, 0.18); min-height: 300px; position: relative; overflow: hidden; cursor: pointer; text-align: left; box-sizing: border-box; transition: all 0.3s ease;">
